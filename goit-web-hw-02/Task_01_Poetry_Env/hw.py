...
class ContactCardView(UserView):
    def display(self, data):
        # Implement logic to display contact details (name, phones, birthday)
        print(f"Contact name: {data.name.value}")
        print(f"Phones: {'; '.join(p.value for p in data.phones)}")
        if data.birthday:
...
def show_all(book):
    for record in book.data.values():
        ContactCardView().display(record)  # Use ContactCardView to display each contact
