...
from src.database.models import Contact  # Assuming this is the model for contacts
...
async def update_contact(body: ContactUpdate, id: int, db: Session = Depends(get_db)):
    ...
    Update a contact.
    """
    contact = await repository_contacts.update_contact(id, body, db)
    if contact is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Contact not found")
    return contact
...
async def remove_contact(id: int, db: Session = Depends(get_db)):
    ...
    Delete a contact.
    """
    contact = await repository_contacts.remove_contact(id, db)
    if contact is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Contact not found")
    return contact
...
async def read_contact_fname(first_name: str, db: Session = Depends(get_db)):
    ...
    """
    contact = await repository_contacts.get_contact_fname(first_name, db)
    if contact is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Contact not found")
    return contact
